...
----------------------------------  
  ...
  **Домашнее задание (от 23.10 до 31.10)**:  
    1) Разработать список 04 из 2 упражнений из **файла docs/list04_dz.pdf**  
      (разработать исходный код + скриншот с 1 тестами в отчете + публикация на GitHub)                    
    2) Указатель: Проведите эксперименты с debugger и/или терминалом, используя разные значения переменных и проверьте обновленные значения переменных (задачи 4, 4.a, 5) из файла **docs/lab2.cpp**.  
...
```
  ...
  **код синтаксис**  
  ...
  Пример:  
```
void name_procedure(double input_param_1, ..., float input_param_n, double *out_param_1, ..., int *out_param_m){
    // набор утверждений      
}  
```
